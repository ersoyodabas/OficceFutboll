...
function handleMessage(msg) {
  ...
  if (msg.type === SERVER.LOBBY) {
    ...
    if (state.phase === 'lobby' || state.phase === 'countdown' || state.waitingInLobby) {
      if (!isMatchPhase(state.phase)) goalSequence.clear();
    ...
    state.serverClockOffset = msg.startedAt - Date.now();
    ui.hideCountdownOverlay();
    state.phase = 'playing';
    ui.showOverlay(null);
  ...
  } else if ([SERVER.STATE, SERVER.GOAL, SERVER.KICKOFF_RESET].includes(msg.type)) {
    if (!state.joined) return;
    ...
    if (typeof msg.serverTime === 'number') state.serverClockOffset = msg.serverTime - Date.now();
    ...
    applyState(msg, transition.snap);
...
return { handleMessage, clearGoalSequence: goalSequence.clear };
}
